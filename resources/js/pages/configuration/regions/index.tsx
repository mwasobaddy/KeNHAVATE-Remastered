...
import { ArrowLeft, SquarePen, Plus, Search, MapPin, Trash2 } from 'lucide-react';
...
import routes from '@/routes/regions';
...
export default function RegionIndex({ regions, filters: initialFilters, search: initialSearch }: Props) {
    ...
    const handleDelete = (region: Region) => {
        if (confirm(`Are you sure you want to delete "${region.name}"?`)) {
            router.delete(routes.destroy({ region: region.id }));
        }
    };
 ...
 setTipBack(true); goBack(); 
                    ...
                    {permissions.includes('region.create') && (
                        <div className="flex flex-col items-center gap-1">
                            <Tooltip open={tipNew} onOpenChange={setTipNew}>
                                ...
                                <TooltipContent>New Region</TooltipContent>
                            </Tooltip>
                            <span className="text-[10px] leading-tight text-muted-foreground text-center">New Region</span>
                        </div>
                    )}
                </div>
                ...
                <Heading
                    title="Regions"
                    description="Manage geographic regions within the organization"
                />
                                        ...
                                        regions.data.map((region) => (
                                            <tr key={region.id} className="border-b last:border-0">
                                                ...
                                                <td className="py-3 pr-4 text-muted-foreground">
                                                    {region.created_by?.name ?? '—'}
                                                </td>
                                                <td className="py-3 pr-4">
                                                    <div className="flex items-start gap-2">
                                                        {permissions.includes('region.edit') && (
                                                            <div className="flex flex-col items-center gap-1">
                                                                <Tooltip open={activeTips[`${region.id}-edit`] ?? false} onOpenChange={(o) => setActiveTips((p) => ({ ...p, [`${region.id}-edit`]: o }))}>
                                                                    <TooltipTrigger asChild>
                                                                        <Button variant="success" size="icon" asChild>
                                                                             <Link href={routes.edit({ region: region.id })} onClick={() => setActiveTips((p) => ({ ...p, [`${region.id}-edit`]: true }))}>
                                                        ...
                                                        {permissions.includes('region.delete') && (
                                                            <div className="flex flex-col items-center gap-1">
                                                                <Tooltip open={activeTips[`${region.id}-delete`] ?? false} onOpenChange={(o) => setActiveTips((p) => ({ ...p, [`${region.id}-delete`]: o }))}>
                                                                    <TooltipTrigger asChild>
                                                                        <Button
                                                                             variant="destructive"
                                                                             size="icon"
                                                                             onClick={() => {
                                                                                 setActiveTips((p) => ({ ...p, [`${region.id}-delete`]: true }));
                                                                                 handleDelete(region);
                                                                             }}
                                                                         >
                                            ...
                                            <td colSpan={5} className="py-12 text-center text-muted-foreground">
                                                {hasSearch || hasActiveFilters ? (
                                                    <div className="flex flex-col items-center gap-2">
                                                        ...
                                                        <p>No regions match your search.</p>
                                                    </div>
                                                ) : (
                                                    <div className="flex flex-col items-center gap-2">
                        ...
                        {regions.last_page > 1 && (
                            <div className="mt-4 flex items-center justify-between">
                                <p className="text-sm text-muted-foreground">
                                    {hasSearch || hasActiveFilters
                                        ? `Showing ${regions.data.length} entries`
                                        : `Showing ${regions.from} to ${regions.to} of ${regions.total} entries`
                                    }
                                </p>
                                {!hasSearch && !hasActiveFilters && (
                                    <div className="flex gap-2">
                                        {regions.links.map((link, i) => {
