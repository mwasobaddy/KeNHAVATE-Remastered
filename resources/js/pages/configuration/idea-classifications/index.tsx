...
import { ArrowLeft, SquarePen, Plus, Search, Tags, Trash2 } from 'lucide-react';
...
import routes from '@/routes/idea-classifications';
...
export default function IdeaClassificationIndex({ idea_classifications, filters: initialFilters, search: initialSearch }: Props) {
    ...
    const handleDelete = (classification: IdeaClassification) => {
        if (confirm(`Are you sure you want to delete "${classification.name}"?`)) {
            router.delete(routes.destroy({ ideaClassification: classification.id }));
        }
    };

    return (
        <>
            <Head title="Idea Classifications" />
 ...
 setTipBack(true); goBack(); 
                    ...
                    {permissions.includes('idea-classification.create') && (
                        <div className="flex flex-col items-center gap-1">
                            <Tooltip open={tipNew} onOpenChange={setTipNew}>
                                ...
                                <TooltipContent>New Classification</TooltipContent>
                            </Tooltip>
                            <span className="text-[10px] leading-tight text-muted-foreground text-center">New Classification</span>
                        </div>
                    )}
                </div>
                ...
                <Heading
                    ...
                    description="Manage classifications for categorizing ideas"
                />
                ...
                <Card>
                    <CardHeader>
                        <CardTitle>All Idea Classifications</CardTitle>
                    </CardHeader>
                    <CardContent>
                        <div className="overflow-x-auto">
                            <table className="w-full text-sm">
                                ...
                                <tbody>
                                    {idea_classifications.data.length > 0 ? (
                                        idea_classifications.data.map((classification) => (
                                            <tr key={classification.id} className="border-b last:border-0">
                                                ...
                                                <td className="py-3 pr-4 text-muted-foreground">
                                                    {classification.created_by?.name ?? '—'}
                                                </td>
                                                <td className="py-3 pr-4">
                                                                ...
                                                                <Tooltip open={activeTips[`${classification.id}-edit`] ?? false} onOpenChange={(o) => setActiveTips((p) => ({ ...p, [`${classification.id}-edit`]: o }))}>
                                                                    <TooltipTrigger asChild>
                                                                        <Button variant="success" size="icon" asChild>
                                                                             <Link href={routes.edit({ ideaClassification: classification.id })} onClick={() => setActiveTips((p) => ({ ...p, [`${classification.id}-edit`]: true }))}>
                                                                ...
                                                                <Tooltip open={activeTips[`${classification.id}-delete`] ?? false} onOpenChange={(o) => setActiveTips((p) => ({ ...p, [`${classification.id}-delete`]: o }))}>
                                                                    <TooltipTrigger asChild>
                                                                        <Button
                                                                             ...
                                                                             size="icon"
                                                                             onClick={() => {
                                                                                 setActiveTips((p) => ({ ...p, [`${classification.id}-delete`]: true }));
                                            ...
                                            <td colSpan={5} className="py-12 text-center text-muted-foreground">
                                                {hasSearch || hasActiveFilters ? (
                                                    <div className="flex flex-col items-center gap-2">
                                                        ...
                                                        <p>No idea classifications match your search.</p>
                                                    </div>
                                                ) : (
                                                    <div className="flex flex-col items-center gap-2">
                                                        <p>No idea classifications found.</p>
                                                    </div>
                                                )}
                                            </td>
                                        </tr>
                        ...
                        {idea_classifications.last_page > 1 && (
                            <div className="mt-4 flex items-center justify-between">
                                <p className="text-sm text-muted-foreground">
                                    {hasSearch || hasActiveFilters
                                        ? `Showing ${idea_classifications.data.length} entries`
                                        : `Showing ${idea_classifications.from} to ${idea_classifications.to} of ${idea_classifications.total} entries`
                                    }
                                </p>
                                {!hasSearch && !hasActiveFilters && (
                                    <div className="flex gap-2">
                                        {idea_classifications.links.map((link, i) => {
...
IdeaClassificationIndex.layout = {
    breadcrumbs: [
